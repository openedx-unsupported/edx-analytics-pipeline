...
FROM (
    ...
    FROM production.production.d_user_course uc
    join production.production.d_course c
    ...
    left outer join (
        select distinct user_id, course_id
        from business_intelligence.production.course_completion_user
    ...
    where lower(c.org_id) not in ('edx')
      ...
      and uc.user_id::varchar || '|' || uc.course_id not in (    -- exclude enterprise enrollments
          select enterprise_user.user_id::varchar || '|' || enroll.course_id
            from app_data.wwc.enterprise_enterprisecourseenrollment enroll
          join app_data.wwc.enterprise_enterprisecustomeruser enterprise_user
...
left outer join (
    ...
    from (
        select uaed.user_id, uaed.course_id, min(uaed.date) first_engaged_date
          from business_intelligence.production.activity_engagement_user_daily uaed
        join production.production.d_course c
...
left outer join (
    ...
    from finance.production.base_transactions bt
    where bt.white_label = false
...
left outer join (
    select ccu.passed_timestamp::date date, count(1) completion_count
      from business_intelligence.production.course_completion_user ccu
    join production.production.d_course c
...
left outer join (
    ...
    where user_id not in (    -- exclude whitelabel-only users
        select user_id
        from app_data.wwc.student_userattribute
