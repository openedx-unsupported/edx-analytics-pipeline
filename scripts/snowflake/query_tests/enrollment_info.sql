SELECT year(enrollment.created) enrollment_year,
  ...
  coalesce(
    case
      when position('-', prod_course.content_language, 1) > 0
        then substring(prod_course.content_language, 1, position('-', prod_course.content_language, 1) -1)
  ...
  sum(case when ccu.passed_timestamp is not null and enrollment.mode in ('verified', 'credit') then 1 else 0 end) verified_passed_count from
            app_data.wwc.student_courseenrollment enrollment join
            production.production.d_user prod_user
   on prod_user.user_id = enrollment.user_id left outer join
  production.production.d_course prod_course
   on prod_course.course_id = enrollment.course_id left outer join
  business_intelligence.production.course_master cm
   on cm.course_id = enrollment.course_id left outer join
  business_intelligence.production.course_completion_user ccu
