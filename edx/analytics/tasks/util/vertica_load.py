...
class VerticaCopyTaskMixin(OverwriteOutputMixin):
    ...
    database = luigi.Parameter(
        default_from_config={'section': 'vertica-export', 'name': 'database'}
    )
...
class VerticaCopyTask(VerticaCopyTaskMixin, luigi.Task):
    ...
    def copy_data_table_from_target(self, cursor, delimiter="E'\t'", null_sequence="'\\N'"):
        """Performs the copy query from the insert source."""
        cursor.copy_file("COPY {table} FROM STDIN DELIMITER AS {delim} NULL AS {null} DIRECT NO COMMIT;"
                         .format(table=self.table, delim=delimiter, null=null_sequence),
