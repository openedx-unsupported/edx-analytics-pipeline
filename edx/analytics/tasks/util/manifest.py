...
def convert_to_manifest_input_if_necessary(manifest_id, targets, manifest_args={}):
    targets = luigi.task.flatten(targets)
    threshold = int(manifest_args.get('threshold', -1))
    if not manifest_args.get('threshold', None):
    ...
    if threshold > 0 and len(targets) >= threshold:
        ...
        return [create_manifest_target(manifest_id, targets, manifest_args)]
    else:
        log.debug(
...
def get_manifest_file_path(manifest_id, manifest_args={}):
    # Construct the manifest file URL from the manifest_id and the configuration
    base_url = manifest_args.get('path', '')
    if not base_url:
...
def create_manifest_target(manifest_id, targets, manifest_args={}):
    ...
    # Construct the manifest file URL from the manifest_id and the configuration
    manifest_file_path = get_manifest_file_path(manifest_id, manifest_args)
    ...
    return ManifestInputTarget.from_existing_targets(targets, manifest_args, *init_args, **init_kwargs)


def remove_manifest_target_if_exists(manifest_id, manifest_args={}):
...
class ManifestInputTargetMixin(object):
    def __init__(self, manifest_args={}, *args, **kwargs):
        super(ManifestInputTargetMixin, self).__init__(*args, **kwargs)

        lib_jar = manifest_args.get('lib_jar', None)
        if not lib_jar:
            lib_jar = configuration.get_config().get(CONFIG_SECTION, 'lib_jar', None)
        input_format = manifest_args.get('input_format', None)
        if not input_format:
            input_format = configuration.get_config().get(CONFIG_SECTION, 'input_format', None)
    ...
    @classmethod
    def from_existing_targets(cls, other_targets, manifest_args={}, *init_args, **init_kwargs):
        manifest_target = cls(manifest_args, *init_args, **init_kwargs)
