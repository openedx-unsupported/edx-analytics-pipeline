...
class UserVideoViewingTask(EventLogSelectionMixin, MapReduceJobTask):
    ...
    def reducer(self, key, events):
        ...
        viewing = None
        counter_headings_for_key = set()
        for event in sorted_events:
            timestamp, event_type, current_time, old_time, youtube_id = event
            parsed_timestamp = ciso8601.parse_datetime(timestamp)
            counter_heading = parsed_timestamp.strftime('Viewings %Y-%m')
            counter_headings_for_key.add(counter_heading)
            ...
            def end_viewing(end_time):
                ...
                if end_time < viewing.start_offset:
                    ...
                    self.incr_counter(counter_heading, 'reject end time before viewing start', 1)
                    return None
                ...
                if (end_time - viewing.start_offset) < VIDEO_VIEWING_MINIMUM_LENGTH:
                    ...
                    self.incr_counter(counter_heading, 'reject end time too close to viewing start', 1)
                    return None
            ...
            if event_type == VIDEO_PLAYED:
                ...
                if event_type in (VIDEO_PAUSED, VIDEO_STOPPED):
                    ...
                    if event_type == VIDEO_PAUSED:
                        self.incr_counter(counter_heading, 'Played to Paused', 1)
                    else:
                        self.incr_counter(counter_heading, 'Played to Stopped', 1)
                elif event_type == VIDEO_SEEK:
                    # play -> seek
                ...
                if viewing_end_time is not None:
                    ...
                    if record:
                        self.incr_counter(counter_heading, 'Total good end-viewing', 1)
                        yield record
                    else:
                ...
                if event_type == VIDEO_PAUSED:
                    ...
                    self.incr_counter(counter_heading, 'paused without preceding played', 1)
                elif event_type == VIDEO_STOPPED:
                    # log.warning('Video stopped event without preceding played event: \nEvent: %r\nKey:%r', event, key)
                    self.incr_counter(counter_heading, 'stopped without preceding played', 1)
                elif event_type == VIDEO_SEEK:
                    ...
                    self.incr_counter(counter_heading, 'seek without preceding played', 1)

        # This happens too often!  Comment out for now...
        if viewing is not None:
            # log.error('Unexpected viewing started with no matching end.\n'
            #           'Viewing Start: %r\nKey:%r', viewing, key)
            self.incr_counter(counter_heading, 'played without matching end', 1)

        for counter_heading in counter_headings_for_key:
            self.incr_counter(counter_heading, 'Distinct event keys', 1)
