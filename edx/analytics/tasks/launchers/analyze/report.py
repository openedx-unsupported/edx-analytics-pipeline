...
def text_report(measurement, file_obj=None, indent=u'', child_indent=u'', threshold_percent=None):
    ...
    for index, child in enumerate(visible_children):
        ...
        text_report(child, file_obj=file_obj, indent=c_indent, child_indent=cc_indent, threshold_percent=threshold_percent)
...
def get_call_graph_svg(measurement, threshold_percent=None):
    ...
    p = subprocess.Popen(['dot', '-Tsvg'], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
    return p.communicate(input=dot_data)[0]
