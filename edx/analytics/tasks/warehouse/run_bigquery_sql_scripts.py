"""
Support for running multiple SQL scripts against an BigQuery database in a deterministic fashion.
"""
...
from edx.analytics.tasks.warehouse.run_bigquery_sql_script import BaseBigQuerySqlScriptTaskMixin, RunBigQuerySqlScriptTask


log = logging.getLogger(__name__)


class RunBigQuerySqlScriptsTaskMixin(BaseBigQuerySqlScriptTaskMixin):
    """
    Parameters for running multiple SQL scripts against an BigQuery database in a deterministic fashion.
    """
...
class RunBigQuerySqlScriptsTask(RunBigQuerySqlScriptsTaskMixin, luigi.WrapperTask):
    """
    A wrapper task for running multiple SQL scripts against an BigQuery database in a deterministic fashion.
    ...
    def validate_script_entry(self, script):
        ...
        for attr in ['name', 'location', 'cli_flags', 'destination_table']:
            ...
            with script_conf_target.open('r') as script_conf_file:
                ...
                if config is not None and isinstance(config, dict):
                    ...
                    for script in scripts:
                        ...
                        new_task = RunBigQuerySqlScriptTask(
                            ...
                            script_name=script.get('name'), cli_flags=script.get('cli_flags'), destination_table=script.get('destination_table'))
