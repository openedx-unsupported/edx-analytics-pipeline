        ...
        with tempfile.NamedTemporaryFile() as temp_config_file:
            ...
            if self.is_remote:
                ...
                command = [
                    '/bin/bash',
                    '-c',
                    '. ~/.bashrc && . /edx/app/analytics_pipeline/venvs/analytics_pipeline/bin/activate && {0} {1} --local-scheduler'.format(os.getenv('LAUNCH_TASK', 'launch-task'),
                                                                      ' '.join(task_args))
                ]
                env = {
                    'LUIGI_CONFIG_PATH': temp_config_file.name
                }
