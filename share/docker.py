...
def list_groups():
    ...
    for host in hosts:
        ...
        for container in containers:
            ...
            for env_setting in inspect.get('Config', {}).get('Env', []):
                var_name, var_value = env_setting.split('=', 1)
                if var_name == 'MR_CLUSTER_ROLES':
                    for role_name in var_value.split(','):
                        groups['mr_{name}_{role}'.format(name=name, role=role_name)].append(name)
